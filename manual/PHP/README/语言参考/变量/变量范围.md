...
在用户自定义函数中，一个局部函数范围将被引入。任何用于函数内部的变量按缺省情况将被限制在局部函数范围内。以下脚本不会有任何输出，因为 `foo()` 函数内返回了一个局部范围的变量 `$foo` ，而且在这个范围内，它并没有被赋值。
...
在局部函数范围内访问全局变量的第二个办法，是用特殊的 `PHP` 超全局变量 `$GLOBALS` 数组。
