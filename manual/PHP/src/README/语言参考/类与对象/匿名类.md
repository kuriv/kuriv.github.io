...
$qux = new class('qux') extends Foo implements Bar
{
    use Baz;
...
匿名类被嵌套进普通 `Class` 后，不能在匿名类中访问这个外部类的 `private` 、 `protected` 方法或者属性。为了访问外部类 `protected` 属性或方法，匿名类可以 `extends` 此外部类。 为了使用外部类的 `private` 属性，必须通过构造函数传进来。
...
/**
 * Get an instance of an anonymous class.
 *
 * @param  void
