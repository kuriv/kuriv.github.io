...
`QUERY_STRING` 是页面的查询字符串，如果有的话，通过它进行页面访问。
...
`HTTP_CONNECTION` 是当前请求头中 `Connection` 项的内容，如果存在的话。
...
`PHP_AUTH_DIGEST` 是当作为 `Apache` 模块运行时，进行 `HTTP Digest` 认证的过程中，此变量被设置成客户端发送的请求头中 `Authorization` 项的内容（以便作进一步的认证操作）。
